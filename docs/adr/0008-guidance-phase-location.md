...
| アプリが背景に回る (`ScenePhase == .background`) | 後述のとおり背景測位を今回は有効化しないため、続けても更新が届かない。**`.inactive` では止めない** — 通知バナー・着信・コントロールセンターで一瞬入るだけの状態で測位を落とすと、画面が見えているのに追従が切れ、再取得の数秒で FR-14 の 3 秒判定を満たせない |
| 経路・地域パッケージの差し替え | 追従対象が無くなる。新しい経路が確定したら改めて開始する |

開始も同じ境界に揃える。**背景にいる間は案内を開始しない。** 経路探索は非同期で、画面を離れた後に完了しうる。そこで発話と連続測位を始めると、見ていない画面のために GPS を焚くことになる (NFR-05)。背景で完了した探索は案内に入らずに捨て、フォアグラウンド復帰時に現在地から引き直す。

判定に `@Environment(\.scenePhase)` を直接読まないのは、その値が View のコピーごとに固定されるためである。探索の Task を作った時点のコピーは `.active` を持ったままで、背景移行が届かない。`@State` の真偽値に写して、走行中の Task からはそちらを見る。
...
- リルートを 3 秒以内に返すには、リルートのたびに `region.sqlite` から `RoadGraph` を読み直すのでは間に合わない可能性がある。読み込み済みのグラフを保持する層 (`RouteEngine`) を UI 層に置く。**読み込みの失敗は記憶しない** — 一過性の I/O 失敗 (ダウンロード直後の書き込み途中のファイルなど) 1 回でそのパッケージの経路探索がアプリ再起動まで死ぬのは、避難経路が出ない状態を復旧不能にすることに等しい。読み込みに失敗している間は案内フェーズに入らない (=連続測位も始まらない) ので、再試行の頻度は平時の位置更新 (significant change) どまりになる
- 進捗案内 (「200メートル先、右に曲がります」) の残距離は **道なり** で測る (`TrackingState.distanceToNextStepM`)。直線距離だと、迂回する経路で実際には数百m以上残っているのに区間の頭で読み上げてしまう。`RouteTracker` は進行の判定に案内地点の道なり距離を既に使っているので、同じ値との差を返すだけでよい
