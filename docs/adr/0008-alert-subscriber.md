...
- 再接続のバックオフは**接続できなかったときだけ伸ばす**。`StableConnection` (既定 30 秒) 以上維持できた接続が切れたら間隔を初期値に戻す。Cloud Run の入れ替えや DMDATA 側の再起動による通常の切断までバックオフの対象にすると、切断を繰り返すうちに間隔が上限 (既定 30 秒) まで伸び、再接続とその直後の ATOM バックフィルが同じだけ遅れる。一方、そもそも繋がらない場合と繋いだ直後に落ちる場合 (資格情報の失効・同時接続数の超過・契約範囲外の購読指定) は再試行を速めても直らず、速めるほど DMDATA の API を叩き続けることになるため、指数バックオフのまま間隔を伸ばす
