...
2. **FR-14 (経路逸脱リルート) / FR-16 (目的地到達検知)** — **実装完了、実機確認が未了** (ADR-0007/ADR-0008)。ドメイン層は `RouteTracking.swift` (追従判定) + `GuidanceNarration.swift` (発話とリルートの判断)、UI 層は `GuidanceSession` (追従と発話の配線) + `RouteEngine` (グラフを保持してリルートを 3 秒に収める) + `RegionCacheCoordinator.beginGuidance/endGuidance` (案内フェーズ中だけ連続測位)。残:
   ...
   - **都市部の実データでの案内過多の確認** (ADR-0007 追記)。進捗案内は区間長 600m 以上に限って抑えてあるが、都市部メッシュのパッケージが手元に無く未検証。パッケージが揃ったら案内件数と区間長の分布を実測して `NarrationStyle.minLegLengthForCueM` と `GuidanceStyle.junctionDegree` を見直す
   - **バックグラウンドでの案内継続は未対応** (ADR-0008)。`UIBackgroundModes` の `location`/`audio` と Always 権限は FR-10 (EEW プッシュでの BG 起動) と一体で決める
   - 案内フェーズの起点は暫定で「実データの経路が確定した瞬間」。電文受信 (#24) が入ったら `EvacuationPhase` の `guidance` 遷移に移す (ADR-0008 の見直し条件)
...
4. **東京都・福井県・香川県の津波浸水想定区域データの補完** — **福井県は実装済み** (`normalize-fukui.sh`、ADR-0003 追記 2026-07-28、敦賀メッシュで動作確認済み)。**東京都・香川県のラスタ→ポリゴン変換も実装済み** (`fetch-tsunami-raster.sh` / `normalize-raster-tsunami.sh`、ADR-0003 追記 2026-09-08。`make pipeline-fetch-raster` → `make pipeline-normalize-data`)。zoom (z14)・色の許容誤差 (RGB 距離 16 + アルファ 128)・PDL1.0 表記は決着済みで、RGB 表の照合はスクリプトの自己検査 (未一致率 > 5% で警告) に組み込んだ。**残: 実配信元 (disaportaldata.gsi.go.jp) での初回実行** — 実装セッションの環境からは同ホストへ到達できず、検証は合成タイルサーバのみ (3 形式 × e2e で真値矩形を 1 画素未満の誤差で再現)。ネットワークのある環境で実行し、(a) `data/<name>/coverage.txt` で東京都本土部のカバレッジ、(b) 未一致率の警告の有無 (= RGB 表が実タイルと合っているか)、(c) 配信タイルの実フォーマットと z14 の配信有無を確認する
