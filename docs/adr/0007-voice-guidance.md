...
## 追記 (2026-09-08): 残課題のうち「無案内区間」を進捗案内で埋めた

FR-14/FR-16 の UI 配線 ([ADR-0008](0008-guidance-phase-location.md)) に合わせて、本 ADR が「残る課題」に挙げた 2 点を扱った。

### 3,000m の無案内区間 → 進捗案内で埋めた

案内の進行は現在地への追従 (`RouteTracker`) から取れるようになったので、区間の残距離が縮むたびに**同じ指示を残距離で読み直す**進捗案内を足した (`GuidanceNarration`)。釜石の 3,000m 区間なら「1キロ先、右に曲がります」→「500メートル先、右に曲がります」→「200メートル先、右に曲がります」と 3 回挟まる。

文言を新造せず `GuidanceScript.text(for:distanceM:)` を残距離で呼び直しているのは、**同じ角を案内地点と進捗で違う言い回しにしないため**。そのために `text(for:distanceM:style:)` を public にした。

### 都市部での案内過多 → 設計で抑えたが、実データ未確認のまま

進捗案内は**区間長が `NarrationStyle.minLegLengthForCueM` (既定 600m) 以上のときだけ**出す。格子状の街路は分岐点が密で区間が短く、そこへ進捗案内を重ねると「曲がる案内 → すぐ進捗案内 → また曲がる案内」と喋り続けることになる。長い無案内区間を埋めるのが目的なので、短い区間では黙る。

#### なぜ 600m か

進捗案内のしきい値は `NarrationStyle.progressCuesM = [1000, 500, 200]` で、`progressCue` は**区間長より短いしきい値しか使わない** (区間の頭で即座に読み上げてしまうため)。つまり区間長が実際に出る案内の本数を決める。600m は「**最大でも 500m / 200m の 2 回**に収まり、かつ 500m の案内が区間の頭に張り付かない」下限として置いた値である。

| 候補 | 出る案内 | 却下の理由 |
|---|---|---|
| 300m | 200m の 1 回 | 都市部の街区は 100〜200m 級で、300m 超の区間はそれなりに現れる。曲がる案内と進捗案内が交互に鳴る状態を作りやすい |
| **600m (採用)** | 500m・200m の 2 回 | 長い無案内区間を埋める目的を満たしつつ、都市部の典型的な街区長からは十分離れている |
| 1000m | 1000m・500m・200m の 3 回 | 1,000m 未満の区間が無案内のまま残る。高台へ登る山道は 600〜1,000m の直進が普通にあり、ADR が埋めようとした穴が残る |

**この 600m は暫定値である。**根拠は街路の一般的な寸法と上記のしきい値の組み合わせだけで、実データの区間長分布には当たっていない。

**見直しの条件**: 都市部メッシュのパッケージが手に入ったら、(1) 1 経路あたりの発話件数、(2) 区間長の分布、(3) 進捗案内が出た区間の割合を実測する。発話件数が本 ADR 本文の基準 (釜石で曲がる案内 3 件) に対して体感で詰まるようなら `minLegLengthForCueM` を引き上げ、逆に無案内区間が残るなら下げる。`GuidanceStyle.junctionDegree` も同じ実測で見直す。

**ただし、都市部メッシュの実データでの確認は依然として未着手である。** 現在ローカルにあるのは釜石メッシュ (584177) の同梱サンプルだけで、都市部のパッケージが無い。本 ADR の「約 90 件 → 3 件」がそうであったように、**合成グラフのテストでは実データの性質を再現できない**。

### 読み仮名 (`name_kana`) は未着手のまま

国土地理院の指定緊急避難場所データにふりがな列があるかは未確認で、状況は変わっていない。
