...
- **本 ADR に基づく実装は、実 DMDATA 接続・実電文のいずれでも未検証である。** 検証はフェイクの WS サーバと手書き fixture によるテストに留まる。以下を満たすまで本番投入しない:
  ...
  3. **API キーの渡し方を確定する**。本実装はクエリ `?key=` を使っているが、DMDATA は `Authorization: Basic <base64(APIキー + ":")>` を推奨している可能性がある (レビューで指摘。未照合)。ヘッダ方式ならキーが URL に載らず、アクセスログや中継に残らないため望ましい。**照合できるまではクエリ方式を維持し、エラーから URL を落とす対処 (`redactURL`) で漏洩経路を塞いでいる**
  4. 実電文サンプルでパーサを検証する
