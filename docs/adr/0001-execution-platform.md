...
- Cloud Run のインスタンスは予告なく入れ替わりうる。WS 切断 → 自動再接続 → 電文の取りこぼし検知をアプリケーション層で必ず実装する (当初はシーケンス番号確認を想定していたが、**取りこぼし検知の具体的な方法は [ADR-0008](0008-alert-subscriber.md) で電文同一性によるデデュープ + ATOM バックフィルに置き換えた**。DMDATA の WS 配信にストリーム全体の連番が載る一次情報を確認できなかったため)
...
- WS 再接続・重複排除・取りこぼし検知はアプリケーション層の必須実装として Stage 1 から組み込む。取りこぼし検知の方法はシーケンス検証ではなく、電文同一性によるデデュープ + 再接続時の ATOM バックフィルである ([ADR-0008](0008-alert-subscriber.md)。上記「注意点」の置き換えと同じ理由)