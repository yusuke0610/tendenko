...
2. **FR-14 (経路逸脱リルート) / FR-16 (目的地到達検知)** — 音声案内 (FR-13) は **実装済み** (ADR-0007。`GuidanceScript` + `SpeechAnnouncer`)。現状は経路確定時に概要と最初の指示を読み上げるところまでで、**現在地に追従して残りの指示を順次発話する部分が未実装**。`GuidanceStep.nodeID` に現在地を突き合わせる形で実装する。ADR-0007 の追記に残課題を記録済み: 実データでは 3,000m の無案内区間が生じるため区間途中の進捗案内が要る、都市部 (分岐点が密) での案内過多は未確認
