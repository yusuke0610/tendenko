...
**ただし、都市部メッシュの実データでの確認は依然として未着手である。** 現在ローカルにあるのは釜石メッシュ (584177) の同梱サンプルだけで、都市部のパッケージが無い。本 ADR の「約 90 件 → 3 件」がそうであったように、**合成グラフのテストでは実データの性質を再現できない**。都市部パッケージが手に入った時点で、案内件数と区間長の分布を実測して `minLegLengthForCueM` と `GuidanceStyle.junctionDegree` を見直す。
